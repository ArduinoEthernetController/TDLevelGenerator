...
@Component({
    ...
    `
        <div class="gamenode-frame" [ngStyle]="node.nodeStyle">
            <div class="gamenode" [ngClass]="node.nodeClasses()" (mouseover)="handleHover()" (click)="handleClick()">{{node.id}}</div>
        </div>
    `
})
...
export class GameNodeComponent {
    ...
    handleClick() { this.onClicked.emit(this.node) }
    handleHover() { this.onHover.emit(this.node) }
}