...
export class GameBoard {
    ...
    scanAround(node: GameNode, edges: boolean, callback) {
        ...
        if (edges)
            scanOffsets = scanOffsets.concat(edgesOffsets);
    ...
    resetBuildzones() {
        this.iterateAll((node: GameNode) => {
            if (node.nodeType == GameNodeType.BUILDZONE)
                node.nodeType = GameNodeType.NORMAL;
        });
    }
            ...
            route.nodes.forEach((node: GameNode) => {
                if (node.nodeType != GameNodeType.START && node.nodeType != GameNodeType.END) {
                    this.scanAround(node, true, (neighbour: GameNode) => {
                        if (neighbour.nodeType == GameNodeType.NORMAL)
                            neighbour.nodeType = GameNodeType.BUILDZONE;
                    });
                }
            });
        }
