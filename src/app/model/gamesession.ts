...
export class GameSession {
  ...
  init(sessionKey: string) {
    if (sessionKey) {
      ...
      if (data) {
        let info: GameSessionData = JSON.parse(atob(data));
  ...
  save() {
    let info = {
      level: this.board.level.map((row: any[]) => {
        return row.map((node: GameNode) => node.nodeType);
      }),
      settings: this.settings
    };
    sessionStorage[this.settings.sessionKey] = btoa(JSON.stringify(info));
  }
}
